...
const getAllMaster = async () => {
  ...
  //   get message master
  const getMessageMaster = await sequelize.query(`SELECT * FROM TBL_MESSAGE_MASTER WHERE STATUS=1`);

  //   get over weight master
  const getOverWeight = await sequelize.query(`SELECT * FROM TBL_OW_MASTER WHERE STATUS=1`);
  ...
  //   get module master
  const getModuleMaster = await sequelize.query(`SELECT * FROM admin_master_modules`);

  //   get report master
  const getReportMaster = await sequelize.query(`SELECT * FROM admin_master_reports`);

  //   get roles master
  const getRolesMaster = await sequelize.query(`SELECT * FROM admin_master_roles`);

  //   get shifts master
  const getShiftMaster = await sequelize.query(`SELECT * FROM admin_master_shifts`);

  //   get payment type master
  const getPaymentTypeMaster = await sequelize.query(`SELECT * FROM PAYMENTTYPE WHERE STATUS=1`);

  // get sub payment type master
  const getPaymentSubTypeMaster = await sequelize.query(`SELECT * FROM PAYMENTSUBTYPE WHERE STATUS=1`);
  // get sub payment type master
  const getCollector = await sequelize.query(`SELECT username,admin_users.id FROM [admin_users]
  inner join admin_logins on admin_logins.user_id=admin_users.id
where role_id=3`);
  // get sub payment type master
  const getCashier = await sequelize.query(`SELECT username,admin_users.id FROM [admin_users]
  inner join admin_logins on admin_logins.user_id=admin_users.id
  where role_id=7 `);
  ...
  return {
    ...
    messageMaster: getMessageMaster[0],
    overWeight: getOverWeight[0],
