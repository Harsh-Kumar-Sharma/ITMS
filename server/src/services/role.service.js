...
const createRole = async (record) => {
  ...
  const existsRole = await db.admin_master_roles.findOne({
    where: {
  ...
  for (const moduleData of roleModules) {
    ...
    const module = await db.admin_master_modules.findOne({ where: { id: moduleData.module_id } });
    if (!module) {
  ...
  const roleRes = await db.admin_master_roles.create({
    role_name: record.role_name,
  ...
  for (const moduleData of roleModules) {
    ...
    await db.admin_role_module_mappings.create({
      role_id: roleId,
      module_id: moduleId,
...
const getRoles = async () => {
  const allroles = await db.admin_master_roles.findAll({
    include: [{
      model: db.admin_role_module_mappings,
      attributes: ['id', 'role_id', 'module_id', 'permission'],
      include: [{ model: db.admin_master_modules, attributes: ['id', 'module_name'] }],
    }]
  });
  ...
  for (let role of allroles) {
    roles.push({
      ...
      modules: role.admin_role_module_mappings.map(m => {
        return {
...
const getRoleById = async (id) => {
  const role = await db.admin_master_roles.findOne({ where: { id } });
  if (!role) {
...
const updateRole = async (roleId, updatedRecord) => {
  // Role Existence Check
  const existingRole = await db.admin_master_roles.findOne({ where: { id: roleId } });
  if (!existingRole) {
    throw new Error(`Role with ID ${roleId} does not exist`);
  }

  // Check if role with same name already exists
  const existsUsername = await db.admin_master_roles.findOne({
    where: {
  ...
  if (updatedRecord.role_name) {
    await db.admin_master_roles.update(
      {
        role_name: updatedRecord.role_name,
        updated_at: new Date().toISOString(),
      },
  ...
  if (updatedRecord.modules && Array.isArray(updatedRecord.modules)) {
    ...
    await db.admin_role_module_mappings.destroy({ where: { role_id: roleId } });
...
const deleteRole = async (roleId) => {
  const role = await db.admin_master_roles.findOne({ where: { id: roleId } });
  if (!role) {
    throw new Error('Role not found');
  }

  //Check if the role is assigned to any user
  const roleAssignment = await db.admin_user_role_mappings.findOne({ where: { role_id: roleId } });
  if (roleAssignment) {
    throw new Error('Cannot delete role. It is assigned to a user.');
  }

  // Delete the role
  await db.admin_master_roles.destroy({ where: { id: roleId } });
  await db.admin_role_module_mappings.destroy({ where: { role_id: roleId } });
  return true;
};
...
const getPermissionByRoleId = async (id) => {
  const permissions = await db.admin_master_roles.findOne({
    where: { id: id },
    include: [
      {
        model: db.admin_role_module_mappings,
  ...
  permissions.admin_role_module_mappings.forEach((moduleMapping) => {
    const module = {
...
const getAllModulesAndSubmodules = async () => {
  ...
  const modules = await db.admin_master_modules.findAll();
