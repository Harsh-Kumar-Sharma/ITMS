<template>
  <div class="row">
            ...
            <div class="fv-row mb-5 row">
              ...
              <div class="col-7">
                <el-select v-model="reportType" @change="clearTable">
                  ...
                  <el-option key="4" label=" CountWithRevanue-Wise" value="CountWithRevanue-Wise" />
                </el-select>
              </div>
            </div>
    ...
    <div v-if="reportType == 'PaymentType-Wise'" class="row p-6" style="margin-left:14px">
      <tableRevenueVue :transactions="data" />
    </div>
    <div v-else-if="reportType == 'LaneAndDate-Wise'" class="row p-6" style="margin-left:14px">
      <dateWiseTable :transactions="data" />
    </div>
    <div v-else-if="reportType == 'CountWithRevanue-Wise'" class="row p-6" style="margin-left:14px">
      <countWithRevanueVue :transactions="data" />
    </div>
...
import tableRevenueVue from "./tableRevenue.vue";
...
import countWithRevanueVue from "./countWithRevanue.vue";
  ...
  methods: {
    async generateReports() {
      ...
      try {
        if (this.fromDate != null && this.toDate != null) {
          ...
          else if (this.reportType === "CountWithRevanue-Wise") {
            payload.reportsId = [5];
          }
          else {
            payload.reportsId = [6];
    ...
    async search() {
      ...
      try {
        ...
        const res = await axios.post(`${BASE_URL}${API_ROUTES.admin_REPORT.GET_REVENUE}`, {
          ...payload,
        });
