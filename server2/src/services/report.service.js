...
const generateReports = async (filterBody) => {
  const report = await db.admin_reports.create({
...
const excelDataFilter = async (filterBody, reportData) => {
  ...
  for (let i = 0; i < file; i++) {
    ...
    await db.admin_report_links.create({
...
WHERE
 ${condition}
   TS.PAYMENT_TYPE IS NOT NULL AND
   PS.STATUS = 1
...
const getCountLaneAndDateWiseReport = async (filterBody, reportData) => {
 ...
 `)
    ...
    const Operater = await sequelize.query(`SELECT COMPANY_NAME FROM admin_company_master where CLIENT_ID=1 AND STATUS=1`)
    const Collection = await sequelize.query(`SELECT COMPANY_NAME FROM admin_company_master where CLIENT_ID=4 AND STATUS=1`)
...
INNER JOIN PAYMENTTYPE AS PS ON TS.PAYMENT_TYPE = PS.PAYMENTTYPE
WHERE ${condition} 1=1
  AND TS.PAYMENT_TYPE IS NOT NULL
  AND PS.STATUS = 1
GROUP BY PS.DESCRIPTION
ORDER BY PAYMENT_TYPE;
`)
...
const getAllReports = async () => {
  const allReports = await sequelize.query(` select * from  [admin_report_links]
  inner join [admin_reports] on [admin_reports].id = [admin_report_links].report_id
  inner join [admin_master_reports] on [admin_master_reports].id= [admin_report_links].[type_id] 
  where download_Status=1 ORDER BY admin_reports.id DESC`);
  return allReports[0];
};
...
const deleteFile = async () => {
  ...
  const data = await sequelize.query(`select  FORMAT(created_at, 'M/d/yyyy, hh:mm:ss') AS created_at , download_url from  [admin_report_links]
inner join [admin_reports] on [admin_reports].id = [admin_report_links].report_id
inner join [admin_master_reports] on [admin_master_reports].id= [admin_report_links].[type_id] where download_Status=1 `)
  ...
  UPDATE admin_report_links
  SET download_Status = 0
  FROM admin_report_links
  INNER JOIN [admin_reports] ON [admin_reports].id = [admin_report_links].report_id
  INNER JOIN [admin_master_reports] ON [admin_master_reports].id = [admin_report_links].[type_id]
...
const reportShiftCollection = async (body, reportData) => {
  try {
    ...
    if (body.collectorID && body.collectorID !== 'null' && body.collectorID !== 'All') {
      const name = await sequelize.query(`	select username from admin_logins where user_id=${collectorID}`)
      writer.write(['        ', '        ', '        ', `Collecter Name : ${name[0][0]}`]);
    }
    if (body.cashierID && body.cashierID !== 'null' && body.cashierID !== 'All') {
      const name = await sequelize.query(`	select username from admin_logins where user_id=${cashierID}`)
